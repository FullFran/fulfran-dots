...
# Detects nix system string and home-directory layout for the current host.

detect_system() {
  uname_s=$(uname -s 2>/dev/null || printf 'Linux')
  uname_m=$(uname -m 2>/dev/null || printf 'x86_64')
  case "$uname_m" in
...
home_prefix=$(detect_home_prefix)
...
{
  home.username = "${user}";
  home.homeDirectory = "${home_prefix}/${user}";
...
if ! grep -q '# FULFRAN_TUI_INSERT_HERE' flake.nix; then
  ...
  # Emits a `mkHome` call so each host carries its own `system` string —
  # this keeps the template portable across linux and darwin without a
  # global hardcoded system.
  _patch_with_python() {
    python3 - "$1" "$2" "$3" "$4" <<'PY'
import sys, pathlib
u, h, preset, system = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
p = pathlib.Path("flake.nix")
src = p.read_text()
entry = (
    f'        "{u}@{h}" = mkHome "{system}" [\n'
    f'          fulfran-dots.presets.{preset}\n'
    f'          ./hosts/{h}.nix\n'
    f'        ];\n'
...
PY
  ...
  _patch_with_sed() {
    # sed fallback — uses a different delimiter (|) to avoid escaping slashes.
    sed -i.bak "s|# FULFRAN_TUI_INSERT_HERE|        \"${user}@${host}\" = mkHome \"${system}\" [\n          fulfran-dots.presets.${preset}\n          ./hosts/${host}.nix\n        ];\n        # FULFRAN_TUI_INSERT_HERE|" flake.nix
    rm -f flake.nix.bak
  }

  if have python3; then
    _patch_with_python "$user" "$host" "$preset" "$system" || _patch_with_sed
  else
    _patch_with_sed
  fi
